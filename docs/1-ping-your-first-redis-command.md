...
### RESP Type Indicators (First-Byte Prefixes)

In RESP, the very first byte of every transmission is a **type prefix indicator** that unambiguously tells the parser how to read and decode the payload:

| Indicator | RESP Data Type    | Wire Example                                       | Decoded Value                       | Role / Usage                                                                                                                        |
| :-------: | :---------------- | :------------------------------------------------- | :---------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------- |
|    `+`    | **Simple String** | `+OK\r\n`<br>`+PONG\r\n`                           | `"OK"`<br>`"PONG"`                  | Fast status/success replies. Cannot contain `\r` or `\n`.                                                                           |
|    `-`    | **Simple Error**  | `-ERR unknown command 'FOO'\r\n`                   | Error string                        | Server error messages. Starts with error category (e.g. `ERR`, `WRONGTYPE`).                                                        |
|    `:`    | **Integer**       | `:1000\r\n`<br>`:-1\r\n`                           | `1000`<br>`-1`                      | 64-bit signed numbers (e.g. counters from `INCR`, lengths from `LLEN`).                                                             |
|    `$`    | **Bulk String**   | `$5\r\nhello\r\n`<br>`$-1\r\n`                     | `"hello"`<br>`nil` (null)           | Binary-safe data with byte-count header. `$-1\r\n` indicates a Null Bulk String.                                                    |
|    `*`    | **Array**         | `*2\r\n$4\r\nECHO\r\n$5\r\nhello\r\n`<br>`*-1\r\n` | `["ECHO", "hello"]`<br>`nil` (null) | Ordered list of RESP elements. Used for client commands and multi-value returns (`MGET`, `KEYS`). `*-1\r\n` indicates a Null Array. |
...
| **Integer (`:`)**       | Starts with `:`, signed 64-bit number, ends with `\r\n`           | `fmt.Sprintf(":%d\r\n", num)`             |
| **Array (`*`)**         | Starts with `*`, count of elements, followed by each element      | `fmt.Sprintf("*%d\r\n...", count)`        |
...
> CRLF stands for Carriage Return Line Feed. It is a special sequence of two character codes used in computer systems to mark the end of a line of text and move the cursor to a new line
